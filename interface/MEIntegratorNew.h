...
//  compile with: > scram b  -j 6 USER_CXXFLAGS="-lopenloops\ -lbar\ -lcoli\ -lopenloops_ppHtt_1tL\ -lpphttxcallme2born\ -lppttxbbxcallme2born\ -lTopQuarkAnalysisSingleTop"
//
//
//
...
#include "TopQuarkAnalysis/SingleTop/interface/CPPProcess_ub_thd.h"
#include "TopQuarkAnalysis/SingleTop/interface/CPPProcess_bdx_thux.h"
#include "TopQuarkAnalysis/SingleTop/interface/CPPProcess_uxbx_txhdx.h"
...
#include "TopQuarkAnalysis/SingleTop/interface/rambo.h"
...
class MEIntegratorNew {
 ...
 public:
  ...
  void   createMash();
  double        probabilitySL4j3b(const double*, int) const;
...
/*   double        probabilitySLUnconstrained(const double*, int ) const; */
  ...
  void   setNormFormulas(TString, TString, TString, TString, TString, TString, TString);
  ...
  pair<double,double> getLFQEnergyCI (float);
  ...
  vector<double> meSquaredOpenLoops_tH      (TLorentzVector*, TLorentzVector*, TLorentzVector*, double&, double& ) const;
  vector<double> meSquaredOpenLoops_ttbb (TLorentzVector*, TLorentzVector*, TLorentzVector*, TLorentzVector*, double&, double&) const;
  vector<double> meSquaredOpenLoops_ttj (TLorentzVector*, TLorentzVector*, TLorentzVector*, double&, double&) const;
  ...
  double qbPdf              ( double, double, int, double) const; 
  double qgPdf              ( double, double, double) const; 
  double qxgPdf              ( double, double, double) const; 
...
MEIntegratorNew::~MEIntegratorNew(){
  ...
  if(accSL4j3bFormula_)    delete accSL4j3bFormula_;
...
void MEIntegratorNew::initVersors(int withJetList){
  ...
  if(withJetList==0){
  ...
  else if( withJetList==1 && jets_.size() < 6 ){
    ...
    eLFq_  = (jets_backup_[0].Vect()).Unit();
    ...
    eMEt_  = (jets_backup_[1].Vect()).Unit();
    eBLep_ = (jets_backup_[2].Vect()).Unit();
    eB1_   = (jets_backup_[3].Vect()).Unit();
    eB2_   = (jets_backup_[4].Vect()).Unit();
    eLFq_  = (jets_backup_[5].Vect()).Unit();
  }
  ...
  //th case 4j3b
    ...
    int iBLep  = withJetList/1000;
    if(iBLep>5 || iBLep<2) {
      cout << iBLep << endl;
      cout << "iBLep Error in MEIntegratorNew::initVersors.. return" << endl;
      return;
    }
    int iB1    = (withJetList - iBLep*1000)/100;
    if(iB1>5 || iB1<2 || iB1==iBLep){
      cout << iB1 << endl;
      cout << "iB1 Error in MEIntegratorNew::initVersors.. return" << endl;
      return;
    }
    int iB2    = (withJetList - iBLep*1000 - iB1*100)/10;
    if(iB2>5 || iB2<2 || iB2==iBLep || iB2==iB1){
      cout << iB2 << endl;
      cout << "iB2 Error in MEIntegratorNew::initVersors.. return" << endl;
      return;
    }

    int iLFq    = (withJetList - iBLep*1000 - iB1*100 - iB2*10 ) /1;
    if(iLFq>5 || iLFq<2 || iLFq == iB1  || iLFq == iB2  || iLFq == iBLep ){
      cout << iLFq << endl;
      cout << "iLFq Error in MEIntegratorNew::initVersors.. return" << endl;
      return;
    }
    ...
    newjets_.push_back( jets_backup_[iB1] );
    newjets_.push_back( jets_backup_[iB2] );
    newjets_.push_back( jets_backup_[iLFq] );
    ...
    eMEt_  = (jets_[1].Vect()).Unit();
    eBLep_ = (jets_[2].Vect()).Unit();
    eB1_   = (jets_[3].Vect()).Unit();
    eB2_   = (jets_[4].Vect()).Unit();
    eLFq_ = (jets_[5].Vect()).Unit();

    if(verbose_) cout << "Permutation: " << iBLep << iB1 << iB2 << iLFq << endl;
...
pair<double, double> MEIntegratorNew::getJetEnergyCI(float eta, float Erec, string flavor, float quantile){
  ...
  //cout<<MEIntegratorNew::getJetEnergyCI - Begin"<<endl;
  double Elow  = 0.;
  double Ehigh = 1000.;
  ...
  //cout<<MEIntegratorNew::getJetEnergyCI - End"<<endl;
  return make_pair(Elow,Ehigh);
}

// !!!!!change number of jet in the sequence jets
...
pair<double, double> MEIntegratorNew::getB1EnergyCI( float quantile ){
  return getJetEnergyCI( jets_[3].Eta(), jets_[3].E(), "Heavy", quantile);
}

pair<double, double> MEIntegratorNew::getB2EnergyCI( float quantile ){
  return getJetEnergyCI( jets_[4].Eta(), jets_[4].E(), "Heavy", quantile);
}

pair<double, double> MEIntegratorNew::getLFQEnergyCI( float quantile ){
  return getJetEnergyCI( jets_[5].Eta(), jets_[5].E(), "Light", quantile);
}

pair<double, double> MEIntegratorNew::getW1JetEnergyCI( float quantile ){
  //cout<<MEIntegratorNew::getW1JetEnergyCI Begin"<<endl;
  return getJetEnergyCI( jets_[6].Eta(), jets_[6].E(), "Light", quantile);
  //  cout<<"MEIntegratorNew::getW1JetEnergyCI End"<<endl;
}

pair<double, double> MEIntegratorNew::getW2JetEnergyCI( float quantile ){
  return getJetEnergyCI( jets_[7].Eta(), jets_[7].E(), "Light", quantile);
}

pair<double, double> MEIntegratorNew::getBHadEnergyCI( float quantile ){
  return getJetEnergyCI( jets_[8].Eta(), jets_[8].E(), "Heavy", quantile);
}
...
bool MEIntegratorNew::compatibilityCheck(float quantile, int print, double& mass, double& massLow, double& massHigh){
  ...
  //  cout<<"MEIntegratorNew::compatibilityCheck - Begin"<<endl;
  ...
  h1Low.SetPtEtaPhiM( (boundsH1.first/jets_[3].E())*jets_[3].Pt(), jets_[3].Eta(), jets_[3].Phi(), (boundsH1.first/jets_[3].E())*jets_[3].M()  );
  h2Low.SetPtEtaPhiM( (boundsH2.first/jets_[4].E())*jets_[4].Pt(), jets_[4].Eta(), jets_[4].Phi(), (boundsH2.first/jets_[4].E())*jets_[4].M()  );

  TLorentzVector h1High, h2High;
  h1High.SetPtEtaPhiM( (boundsH1.second/jets_[3].E())*jets_[3].Pt(), jets_[3].Eta(), jets_[3].Phi(), (boundsH1.second/jets_[3].E())*jets_[3].M()  );
  h2High.SetPtEtaPhiM( (boundsH2.second/jets_[4].E())*jets_[4].Pt(), jets_[4].Eta(), jets_[4].Phi(), (boundsH2.second/jets_[4].E())*jets_[4].M()  );
  ...
  //  cout<<"MEIntegratorNew::compatibilityCheck - End"<<endl;
...
// the jet position is not reliable because it has been changed for th channel
...
void MEIntegratorNew::setTopMass(double massTop, double massW){
  ...
  // cout<<"MEIntegratorNew - Entering setTopMass function"<<endl;
...
void MEIntegratorNew::setSumEt(double sumEt){
  ...
  //  cout<<"MEIntegratorNew - Within setSumEt"<<endl;
...
void MEIntegratorNew::setMEtCov(double Vx, double Vy, double Vxy){
  ...
  //  cout<<"MEIntegratorNew - Within setMETCov function"<<endl;
  ...
  cout<<"Within setMETCov function"<<endl;  
...
void MEIntegratorNew::setTopSign(int sign){
  topLepSign_ = sign;
}
...
double MEIntegratorNew::qgPdf( double x1, double x2, double Q) const {

  double lumiQG =  2*(LHAPDF::xfx(0, x1, Q, 1) *  LHAPDF::xfx(0, x2, Q, 0) + 
		      LHAPDF::xfx(0, x1, Q, 2) *  LHAPDF::xfx(0, x2, Q, 0) + 
		      LHAPDF::xfx(0, x1, Q, 3) *  LHAPDF::xfx(0, x2, Q, 0) + 
		      LHAPDF::xfx(0, x1, Q, 4) *  LHAPDF::xfx(0, x2, Q, 0) +
		      LHAPDF::xfx(0, x1, Q, 5) *  LHAPDF::xfx(0, x2, Q, 0)
		      ) 
    /x1/x1/x2/x2;
...
double MEIntegratorNew::qbPdf( double x1, double x2, int f, double Q) const {

  //  cout<<"MEIntegratorNew - qbPdf"<<endl;

  double lumiQB =  (LHAPDF::xfx(0, x1, Q, f) *  LHAPDF::xfx(0, x2, Q, 5)  ) 
    /x1/x1/x2/x2;
...
double MEIntegratorNew::qxgPdf( double x1, double x2, double Q) const {

  double lumiQXG =  2*(LHAPDF::xfx(0, x1, Q, -1) *  LHAPDF::xfx(0, x2, Q, 0) + 
		       LHAPDF::xfx(0, x1, Q, -2) *  LHAPDF::xfx(0, x2, Q, 0) + 
		       LHAPDF::xfx(0, x1, Q, -3) *  LHAPDF::xfx(0, x2, Q, 0) + 
		       LHAPDF::xfx(0, x1, Q, -4) *  LHAPDF::xfx(0, x2, Q, 0) +
		       LHAPDF::xfx(0, x1, Q, -5) *  LHAPDF::xfx(0, x2, Q, 0)
...
double MEIntegratorNew::Eval(const double* x) const {
  ...
  //  cout<<"MEIntegratorNew - Eval"<<endl;
...
/*     prob = probabilitySL2wj(x,0);   */   
  if(intType_ == IntegrationType::SL4j3b){
    prob = probabilitySL4j3b(x,0);     
  }
...
void MEIntegratorNew::setIntType( IntegrationType type ){
  ...
  //  cout<<"MEIntegratorNew -Within setIntType function"<<endl;

  intType_ = type;
}

void MEIntegratorNew::setHypo( int hyp ){
  //  cout<<"MEIntegratorNew - Set Hypo"<<endl;
  hypo_ = hyp;
}

void MEIntegratorNew::setJets( std::vector<TLorentzVector>* jets){

  //  cout<<"MEIntegratorNew - Within setJets function"<<endl;
  resetEvaluation();
...
vector<double> MEIntegratorNew::meSquaredOpenLoops_tH (TLorentzVector* top, TLorentzVector* quark, TLorentzVector* higgs, double& x1, double& x2) const{

  //  cout<<"MEIntegratorNew - Within meSquaredOpenLoops_tH"<<endl;


  TLorentzVector t, q, h;
  t. SetPtEtaPhiM(   top->Pt(),  top->Eta(),   top->Phi(), Mtop_);
  q.SetPtEtaPhiE(  quark->Pt(), quark->Eta(),  quark->Phi(), quark->E());
  h. SetPtEtaPhiM( higgs->Pt(),higgs->Eta(), higgs->Phi(), M_   );

  TLorentzVector vSum = t+q+h;
  TVector3 boostPt( vSum.Px()/vSum.E(), vSum.Py()/vSum.E(), 0.0 );
  ...
  // fix for rounding
  double hPx = -(t.Px()  + q.Px());
  double hPy = -(t.Py()  + q.Py());
  double hPz = h.Pz();
  h.SetPxPyPzE( hPx, hPy, hPz, sqrt(hPx*hPx + hPy*hPy + hPz*hPz + M_*M_) );

  double E  = t.E() +q.E() + h.E();
  double Pz = t.Pz()+q.Pz()+ h.Pz();
  TLorentzVector g1 = TLorentzVector(0.,0.,  (E+Pz)/2., (E+Pz)/2.); 
  TLorentzVector g2 = TLorentzVector(0.,0., -(E-Pz)/2., (E-Pz)/2.); 
  ...
  if(verbose_){
    ...
    cout << "t:  " << t.M() << endl;
    cout << "q:  " << q.M() << endl;
    cout << "q:  "<<q.Pt()<<" "<<q.Phi()<<" "<<q.Eta()<<endl;  
  }


  // !!!!!!  WHAT DOES IT DO?
  // calcola l'elemento di matrice
  //pphttxcallme2born_(const_cast<double*>(&M2), ccP, const_cast<double*>(&Mtop_), const_cast<double*>(&M_));



   // Create a process object
  //  cout<<"MEIntegratorNew: Before process intance"<<endl;



  //  cout<<"MEIntegratorNew - Process.external: "<<process.nexternal<<endl;

  //  cout<<"MEIntegratorNew: After process instance"<<endl;
  ...
  vector<double*> p;
  //  double *x2parton, *topquark, *higgsboson, *lightquark;
  double x1parton[4];
  double x2parton[4];
  double topquark[4];
  double higgsboson[4];
  ...
  p.push_back(x1parton);
  p.push_back(x2parton);
  p.push_back(topquark);
  p.push_back(higgsboson);
  p.push_back(lightquark);

  //  cout<<"MEIntegratorNew - p vector filled: "<<endl;
  //  cout<<"MEIntegratorNew - Process.external: "<<process.nexternal<<endl;
...
/*   cout << " -----------------------------------------------------------------------------" << endl; */
  ...
  CPPProcess_bdx_thux process_bdx;
  CPPProcess_ub_thd process_ub;
  CPPProcess_dbx_txhu process_dbx;
  CPPProcess_uxbx_txhdx process_uxbx;
  const double* matrix_elements_1;
  const double* matrix_elements_2;


  if(topLepSign_==1) {
    
 
    process_ub.initProc("param_card.dat");
    process_ub.setMomenta(p);
    process_ub.sigmaKin();
    matrix_elements_1 = process_ub.getMatrixElements();


    if(verbose_){
      cout<<"*********New implementation for ub_thd process"<<endl;
      for(int i=0;i < process_ub.nexternal; i++)
	cout << setw(4) << i+1
	     << std::scientific << setw(14) << p[i][0]
	     << std::scientific << setw(14) << p[i][1]
	     << std::scientific << setw(14) << p[i][2]
	     << std::scientific << setw(14) << p[i][3] << endl;
      cout << " -----------------------------------------------------------------------------" << endl;
    
      // Display matrix elements
      for(int i=0; i<process_ub.nprocesses;i++)
	cout << " Matrix element = "
	  //	 << setiosflags(ios::fixed) << setprecision(17)
	  //	 << setw(14)
	     << matrix_elements_1[i]
	     << " GeV^" << -(2*process_ub.nexternal-8) << endl;
      
      cout << " -----------------------------------------------------------------------------" << endl;
    }//end of verbose_

    process_bdx.initProc("param_card.dat");
    process_bdx.setMomenta(p);
    process_bdx.sigmaKin();
    matrix_elements_2 = process_bdx.getMatrixElements();

    if(verbose_){
      cout<<"*********New implementation for dxb_thux process"<<endl;
      for(int i=0;i < process_bdx.nexternal; i++)
	cout << setw(4) << i+1 
	     << std::scientific << setw(14) << p[i][0]
	     << std::scientific << setw(14) << p[i][1]
	     << std::scientific << setw(14) << p[i][2]
      ...
      // Display matrix elements
      for(int i=0; i<process_bdx.nprocesses;i++)
	cout << " Matrix element = " 
	  //	 << setiosflags(ios::fixed) << setprecision(17)
	  //	 << setw(14)
	     << matrix_elements_2[i]
	     << " GeV^" << -(2*process_bdx.nexternal-8) << endl;
      
      cout << " -----------------------------------------------------------------------------" << endl;
    }//end of verbose_
    
    
  }//end if top charge is positive
  


  else if(topLepSign_==-1) {


 
    process_uxbx.initProc("param_card.dat");
    process_uxbx.setMomenta(p);
    process_uxbx.sigmaKin();
    matrix_elements_1 = process_uxbx.getMatrixElements();


    if(verbose_){
      cout<<"*********New implementation for uxbx_txhdx process"<<endl;
      
      for(int i=0;i < process_uxbx.nexternal; i++)
	cout << setw(4) << i+1
	     << std::scientific << setw(14) << p[i][0]
	     << std::scientific << setw(14) << p[i][1]
	     << std::scientific << setw(14) << p[i][2]
      ...
      // Display matrix elements
      for(int i=0; i<process_uxbx.nprocesses;i++)
	cout << " Matrix element = "
	  //	 << setiosflags(ios::fixed) << setprecision(17)
	  //	 << setw(14)
	     << matrix_elements_1[i]
	     << " GeV^" << -(2*process_uxbx.nexternal-8) << endl;
      
      cout << " -----------------------------------------------------------------------------" << endl;
   }//end of verbose_
    
    
    process_dbx.initProc("param_card.dat");
    process_dbx.setMomenta(p);
    process_dbx.sigmaKin();
    matrix_elements_2 = process_dbx.getMatrixElements();
    
    if(verbose_){
      cout<<"*********New implementation for dbx_txhu process"<<endl;
      for(int i=0;i < process_dbx.nexternal; i++)
	cout << setw(4) << i+1 
	     << std::scientific << setw(14) << p[i][0]
	     << std::scientific << setw(14) << p[i][1]
	     << std::scientific << setw(14) << p[i][2]
      ...
      // Display matrix elements
      for(int i=0; i<process_dbx.nprocesses;i++)
	cout << " Matrix element = " 
	  //	 << setiosflags(ios::fixed) << setprecision(17)
	  //	 << setw(14)
	     << matrix_elements_2[i]
	     << " GeV^" << -(2*process_dbx.nexternal-8) << endl;
  ...
  M2.push_back(matrix_elements_1[0]);
  M2.push_back(matrix_elements_1[1]);
  M2.push_back( matrix_elements_2[0]);
  M2.push_back( matrix_elements_2[1]);
  

  //  cout<<"MEIntegratorNew:: End ME function"<<endl;

  return M2;

}

vector<double> MEIntegratorNew::meSquaredOpenLoops_ttbb (TLorentzVector* top, TLorentzVector* atop, TLorentzVector* b1, TLorentzVector* b2, double& x1, double& x2) const{

  //  cout<<"MEIntegratorNew - Within meSquaredOpenLoops_ttbb"<<endl;
  ...
  ppttxbbxcallme2born_(const_cast<double*>(&M2_x1x2), ccP_x1x2, const_cast<double*>(&Mtop_), const_cast<double*>(&M_));
  ppttxbbxcallme2born_(const_cast<double*>(&M2_x2x1), ccP_x2x1, const_cast<double*>(&Mtop_), const_cast<double*>(&M_));

  M2.push_back(M2_x1x2);
  M2.push_back(M2_x2x1);
  return M2;

}

vector<double> MEIntegratorNew::meSquaredOpenLoops_ttj (TLorentzVector* top, TLorentzVector* atop, TLorentzVector* quark, double& x1, double& x2) const{

  //  cout<<"MEIntegratorNew - Within meSquaredOpenLoops_ttbb"<<endl;

  TLorentzVector t, tx, q;
  t. SetPtEtaPhiM(   top->Pt(),  top->Eta(),   top->Phi(), Mtop_);
  tx.SetPtEtaPhiM(  atop->Pt(), atop->Eta(),  atop->Phi(), Mtop_);
  q. SetPtEtaPhiM(    quark->Pt(),   quark->Eta(),    quark->Phi(),  0.   );


  TLorentzVector vSum = t+tx+q;
  TVector3 boostPt( vSum.Px()/vSum.E(), vSum.Py()/vSum.E(), 0.0 );
 
  //cout<<"BOOST PT"<<boostPt.Px()<<endl;
  ...
  // fix for rounding
  double qPx = -(t.Px()  + tx.Px() );
  double qPy = -(t.Py()  + tx.Py());
  double qPz = q.Pz();
  q.SetPxPyPzE( qPx, qPy, qPz, sqrt(qPx*qPx + qPy*qPy + qPz*qPz ) );


  if(verbose_){
    cout << "q:  " << q.M() << endl;
    cout << "q Eta:  " << q.Eta() << endl;
    cout << "q Phi:  " << q.Phi() << endl;
    cout << "t:  " << t.M() << endl;
    cout << "t Eta:  " << t.Eta() << endl;
    cout << "t Phi:  " << t.Phi() << endl;
    cout << "tx: " << tx.M() << endl;
    cout << "tx Eta: " << tx.Eta() << endl;
    cout << "tx Phi: " << tx.Phi() << endl;
  }
  

  double E  = t.E() +tx.E() + q.E() ;
  double Pz = t.Pz()+tx.Pz()+ q.Pz();
  TLorentzVector g1 = TLorentzVector(0.,0.,  (E+Pz)/2., (E+Pz)/2.); 
  TLorentzVector g2 = TLorentzVector(0.,0., -(E-Pz)/2., (E-Pz)/2.); 
  ...
  CPPProcess_gg_ttxg process_ttxg;
  ...
  p.push_back(x2parton);
  p.push_back(topquark);
  p.push_back(atopquark);
  p.push_back(lightquark);



  process_ttxg.initProc("param_card.dat");
  process_ttxg.setMomenta(p);
  process_ttxg.sigmaKin();
  matrix_elements = process_ttxg.getMatrixElements();
  
  
  if(verbose_){
    cout<<"*********New implementation for bkg process"<<endl;
    for(int i=0;i < process_ttxg.nexternal; i++)
      cout << setw(4) << i+1
	   << std::scientific << setw(14) << p[i][0]
	   << std::scientific << setw(14) << p[i][1]
	   << std::scientific << setw(14) << p[i][2]
    ...
    cout << " -----------------------------------------------------------------------------" << endl;
      ...
      // Display matrix elements
    for(int i=0; i<process_ttxg.nprocesses;i++)
      cout << " Matrix element = "
	//	 << setiosflags(ios::fixed) << setprecision(17)
	//	 << setw(14)
	   << matrix_elements[i]
	   << " GeV^" << -(2*process_ttxg.nexternal-8) << endl;
  ...
  // !!siamo arrivati qui, va cambiata questa interfaccia
  //  ppttxjcallme2born_(const_cast<double*>(&M2_x1x2), ccP_x1x2, const_cast<double*>(&Mtop_), const_cast<double*>(&M_));
  //  ppttxjcallme2born_(const_cast<double*>(&M2_x2x1), ccP_x2x1, const_cast<double*>(&Mtop_), const_cast<double*>(&M_));


  //  M2.push_back(M2_x1x2);
  //  M2.push_back(M2_x2x1);

  M2.push_back( matrix_elements[0]);
  M2.push_back( matrix_elements[1]);
  return M2;
...
void MEIntegratorNew::setNormFormulas(TString fxSec, TString facc1, TString facc2, TString facc3, TString facc4,TString facc5,  TString fPt){

  //  cout<<"MEIntegratorNew - Enter setNormFomulas function"<<endl;


  xSecFormula_  = new TF1("xSecFormula", fxSec.Data(), 20., 300.);
  xSecFormula_->SetNpx(300);

  // !!!!!! still to adapt to th case 
  accSL4j3bFormula_   = new TF1("accSL4j3bFormula",  facc5.Data() , 20., 300.);
  accSL4j3bFormula_->SetNpx(300);
  ...
  cout << "Acceptance SL4j3b formula:    " << string(accSL4j3bFormula_->GetTitle())   << endl;
...
double MEIntegratorNew::xSection(int flag) const{
  ...
  if( SqrtS_>7999 && SqrtS_<8001 ){
    ...
    else if( (intType_ == SL4j3b ) && accSL4j3bFormula_)
      acc  = accSL4j3bFormula_->Eval(M_) ;
...
//!!! needs to be defined for 4j 3b category for th analysis
double MEIntegratorNew::probabilitySL4j3b(const double* x, int sign) const{

  //cout<<"MEIntegratorNew - Enter probabilitySL4j3b function"<<endl;
  ...
  //The order is the same defined for the integration range definition
  ...
  //nel caso del leptone si assume risoluzione infinita sull'energia, per cui E gen = E reco
  ...
  // We do not have an hadronic t in th channel, so remove it
 ...
 if(verbose_){
    ...
    cout << "eblept (theta, phi) "<<eBLep_.Theta()<<" "<<eBLep_.Phi()<<endl;
  ...
  // check what to do with tt+jets hp

 double cosThetaq2, phiq2;
  if(hypo_==1){
    cosThetaq2   =  x[4];
  ...
  double Eq2, EbHad, cos1Had, cos2Had;
  int nSolTopHad = topHadLostEnergies( phiq2, cosThetaq2, Eq2, Eh1, EbHad, cos1Had, cos2Had, errFlag );
 ...
 if(verbose_){
  ...
  TLorentzVector lfq  ( eLFq_*(Elfq ), Elfq );

  TLorentzVector W1Had( eW1Had_*Eh1, Eh1 );
  ...
  if(hypo_==0){
    tot = topLep+topHad+lfq;
  }
  else if(hypo_==1){
    tot = topLep+higgs+lfq;
  }

    boostToCMS  = tot.BoostVector();
  double x1 = (  tot.Pz() + tot.E() )/SqrtS_;
  double x2 = ( -tot.Pz() + tot.E() )/SqrtS_;

  //To change --> look at Maltoni's LHE file.
  double Q  = (2*Mtop_ + M_)/2.;

  //cout << "x1 (NLO) = " << x1 << ", x2 (NLO) = " << x2 << endl;

  vector<double> me2;
  double me2_x1x2, me2_x2x1;



  /* Be careful because the matrix element is going to be calculated just for the ub>thd channel*/


  // Needs to understand how to treat the background hp
  if(useME_==1 )               // topLep = t,  topHad = tx
    me2 = hypo_==0 ? meSquaredOpenLoops_tH( &topLep,&lfq, &higgs, x1, x2 ) : meSquaredOpenLoops_ttj( &topLep, &topHad, &lfq, x1, x2 );
  ...
  //else if(useME_==1 && top1Flag_ == -1 && top2Flag_ == +1)          // topLep = tx, topHad = t
    // me2 = hypo_==0 ? meSquaredOpenLoops_tH( &lfq, &topLep, &higgs, x1, x2 ) : meSquaredOpenLoops_ttbb( &lfq, &topLep, &higgs1, &higgs2, x1, x2 );
  else{
  ...
  //cout<<"MEIntegratorNew - me computed"<<endl;

  double decayMEpart=
    topLepDensity(cos1Lep,cos2Lep) * 
    (hypo_==0 ? higgsDensity(cos1Higgs) : topHadDensity(cos1Had, cos2Had));

  //cout<<"MEIntegratorNew - Decay part computed"<<endl;

  double Jpart = 
    //topHadJakobi( Eq1,  Eq2, EbHad, &WHad ) * 
    topLepJakobi( Enu, Elep, EbLep, &WLep ) * 
    (hypo_==0 ? higgsJakobi( Eh1, Eh2 ) : topHadLostJakobi(Eh1, Eq2, EbHad, &WHad) ) ;

  //cout<<"MEIntegratorNew - Jacobian computed"<<endl;
  ...
  //Recoiling quark
  double tf2 = jetEnergyTF( jets_[5].Eta(), jets_[5].E(),  lfq.E(),   "Light");
  //B quarks from Higgs
  double tf5 = jetEnergyTF( jets_[3].Eta(), jets_[3].E(),  higgs1.E(),  "Heavy");
  double tf6 = jetEnergyTF( jets_[4].Eta(), jets_[4].E(),  higgs2.E(),  "Heavy");


  double dPx = WLepNu.Px() - jets_[1].Px();
  double dPy = WLepNu.Py() - jets_[1].Py();
  if( constrainToRecoil_ ){
    dPx =  -(jets_[0]+jets_[1]+jets_[2]+jets_[3]+jets_[4]+jets_[5]).Px() + tot.Px() ;
    dPy =  -(jets_[0]+jets_[1]+jets_[2]+jets_[3]+jets_[4]+jets_[5]).Py() + tot.Py() ;
  }
  ...
  //cout<<"MEIntegratorNew - TF computed"<<endl;


  // ub > thd
  // pdf fu(x1)* fb(x2)/x1x2
  double PDFpart1 = qbPdf( x1, x2 ,2, Q);
  // pdf fu(x2)* fb(x1)/x1x2
  double PDFpart2 = qbPdf( x2, x1 ,2, Q);

  // dxb > thux
  // pdf fdx(x1)* fb(x2)/x1x2
  double PDFpart3 = qbPdf( x1, x2 ,-1, Q);
  // pdf fdx(x2)* fb(x1)/x1x2
  double PDFpart4 = qbPdf( x2, x1 ,-1, Q);


  // uxbx > txhdx
  // pdf fu(x1)* fb(x2)/x1x2
  double PDFpart5 = qbPdf( x1, x2 ,-2, Q);
  // pdf fu(x2)* fb(x1)/x1x2
  double PDFpart6 = qbPdf( x2, x1 ,-2, Q);


  // dbx > txhu
  // pdf fdx(x1)* fb(x2)/x1x2
  double PDFpart7 = qbPdf( x1, x2 ,1, Q);
  // pdf fdx(x2)* fb(x1)/x1x2
  double PDFpart8 = qbPdf( x2, x1 ,1, Q);

  double PDF_ttxj = ggPdf(x1, x2, Q);
  ...
  if(useME_ == 1){
    ...
    if(hypo_== 0)
      {
	if(topLepSign_==1)   MEPDFpart =  PDFpart1*me2[0]+PDFpart2*me2[1] + PDFpart3*me2[2] + PDFpart4*me2[3]  ;
	else  MEPDFpart =  PDFpart5*me2[0]+PDFpart6*me2[1] + PDFpart7*me2[2] + PDFpart8*me2[3]  ;
      }

    if(hypo_== 1)
      {
	MEPDFpart =  PDF_ttxj*(me2[0]+me2[1]);
      }

	//    cout<<"MEIntegratorNew - ME + PDF part computed"<<endl;  
  }
  ...
  // !!!build the pedf*ME part, needs to be different from this one
  ...
  if(usePDF_ == 1){
    ...
    if(useME_ ==1)  {
      ...
      if(hypo_==0){
	if(topLepSign_==1)   PDFpart =  PDFpart1+PDFpart2 + PDFpart3 + PDFpart4  ;
	else  PDFpart =  PDFpart5+PDFpart6 + PDFpart7 + PDFpart8  ;
      }
      else PDFpart = PDF_ttxj;
  ...
  //cout<<"MEIntegratorNew - prop assembled"<<endl;  

////////////////////////

  if( TMath::IsNaN(prob) ){
    if(verbose_){
      cout << tf1 << ", " << tf2 << ", " <<  tf5 << ", "<< tf6 << ", " << tf7 << endl;
      cout << me2[0] << ", "   << me2[1] << ", "<< Jpart << ", " << PDFpart1 << ", " << PDFpart2<<endl;
    }
    prob = 0.;
  }
